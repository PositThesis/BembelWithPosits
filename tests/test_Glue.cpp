...
int main() {
  ...
  */
  ...
  Bembel::Geometry<Scalar_> g1("test_EdgeCase1.dat");
  Bembel::Geometry<Scalar_> g2("test_EdgeCase2.dat");
  Bembel::Geometry<Scalar_> g3("test_EdgeCase3.dat");
  Bembel::Geometry<Scalar_> g4("test_EdgeCase4.dat");
  ...
  for (int i = 0; i < 5; ++i) {
    assert(g1.get_geometry()[i].evalNormal(Eigen::Vector2<Scalar_>(.5, .5))(2) > 0);
    assert(g2.get_geometry()[i].evalNormal(Eigen::Vector2<Scalar_>(.5, .5))(2) > 0);
    assert(g3.get_geometry()[i].evalNormal(Eigen::Vector2<Scalar_>(.5, .5))(2) > 0);
    assert(g4.get_geometry()[i].evalNormal(Eigen::Vector2<Scalar_>(.5, .5))(2) > 0);
  }
  ...
  for (int P = 1; P < 5; ++P) {
    for (int M = 0; M < 4; ++M) {
      ...
      std::vector<Scalar_> pts;
      const int elements = (1 << M);
      const Scalar_ h = 1. / Scalar_(elements);
      for (int i = 0; i < elements; ++i) {
        pts.push_back((i + 0.5) * h);
      }
      pts.shrink_to_fit();

      AnsatzSpace<TestOperatorDivC, Scalar_> a1(g1, M, P);
      AnsatzSpace<TestOperatorDivC, Scalar_> a2(g2, M, P);
      AnsatzSpace<TestOperatorDivC, Scalar_> a3(g3, M, P);
      AnsatzSpace<TestOperatorDivC, Scalar_> a4(g4, M, P);

      AnsatzSpace<TestOperatorC, Scalar_> a1c(g1, M, P);
      AnsatzSpace<TestOperatorC, Scalar_> a2c(g2, M, P);
      AnsatzSpace<TestOperatorC, Scalar_> a3c(g3, M, P);
      AnsatzSpace<TestOperatorC, Scalar_> a4c(g4, M, P);
      ...
      Eigen::VectorX<Scalar_> coefsDiv(dofsDiv);
      for (int i = 0; i < dofsDiv; ++i) {
        coefsDiv(i) = i - dofsDiv / 2;
      }
      Eigen::VectorX<Scalar_> coefsCon(dofsCon);
      for (int i = 0; i < dofsCon; ++i) {
        coefsCon(i) = i - dofsCon / 2;
      }

      FunctionEvaluator<TestOperatorDivC, Scalar_> fe1(a1, coefsDiv);
      FunctionEvaluator<TestOperatorDivC, Scalar_> fe2(a2, coefsDiv);
      FunctionEvaluator<TestOperatorDivC, Scalar_> fe3(a3, coefsDiv);
      FunctionEvaluator<TestOperatorDivC, Scalar_> fe4(a4, coefsDiv);

      FunctionEvaluator<TestOperatorC, Scalar_> fe1c(a1c, coefsCon);
      FunctionEvaluator<TestOperatorC, Scalar_> fe2c(a2c, coefsCon);
      FunctionEvaluator<TestOperatorC, Scalar_> fe3c(a3c, coefsCon);
      FunctionEvaluator<TestOperatorC, Scalar_> fe4c(a4c, coefsCon);
      ...
      for (int j = 0; j < pts.size(); ++j) {
        ...
        const Scalar_ pt_p = pts[j];
        const Scalar_ pt_n = 1.0 - pt_p;

        const Eigen::Vector2<Scalar_> e1p(pt_p, 0);
        const Eigen::Vector2<Scalar_> e2p(1, pt_p);
        const Eigen::Vector2<Scalar_> e3p(pt_p, 1);
        const Eigen::Vector2<Scalar_> e4p(0, pt_p);

        const Eigen::Vector2<Scalar_> e1n(pt_n, 0);
        const Eigen::Vector2<Scalar_> e2n(1, pt_n);
        const Eigen::Vector2<Scalar_> e3n(pt_n, 1);
        const Eigen::Vector2<Scalar_> e4n(0, pt_n);
        ...
        const Eigen::Vector3<Scalar_> uv1(1, 0, 0);
        const Eigen::Vector3<Scalar_> uv2(0, 1, 0);
