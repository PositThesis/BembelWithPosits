...
bool test_integrate4(const Bembel::AnsatzSpace<Derived, ptScalar> &ansatz_space,
                     const Bembel::LinearOperatorBase<Derived, ptScalar> &linOp) {
  Bembel::GaussSquare<maxqdeg + 1, ptScalar> GS;
  auto Q = GS[maxqdeg];
  Eigen::MatrixX<ptScalar> ffield_qnodes(0, 0);
  Eigen::MatrixX<ptScalar> intval;
  Eigen::MatrixX<ptScalar> axis;
  intval.resize(1, 1);
  axis.resize(2, 4);
  ...
  for (auto it = begin; it != end; ++it)
    for (auto it2 = begin; it2 != end; ++it2) {
      ...
      if (cp(2) == 4) {
        ...
        auto exactInt =
            integrate4_test_function_integral<ptScalar>(axis) / BEMBEL_SQUARED_(h);
        ////////////////////////////////////////////////////////////////////////
