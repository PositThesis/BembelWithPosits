...
int main() {
  ...
  for (int refinement_level = 0; refinement_level < 11; ++refinement_level) {
    sw.tic();
    Bembel::ClusterTree<Scalar> mesh(geometry, refinement_level);
  ...
  for (int refinement_level = 0; refinement_level < 5; ++refinement_level) {
    ...
    Bembel::AnsatzSpace<Bembel::LaplaceSingleLayerOperator<Scalar>, Scalar> ansatz_space(
        geometry, refinement_level, polynomial_degree);
    Bembel::LaplaceSingleLayerOperator<Scalar> S;
    Bembel::BlockClusterTree<Scalar, Scalar> bct0(S, ansatz_space);
    std::cout << std::setw(2) << refinement_level
              << ") setup time: " << sw.toc() << std::endl;
  }
  std::cout << std::string(60, '-') << std::endl;
  {
    ...
    Bembel::BlockClusterTree<Scalar, Scalar> bct0(S, ansatz_space);
    sw.tic();
    Bembel::BlockClusterTree<Scalar, Scalar> bct =
        std::move(Bembel::BlockClusterTree<Scalar, Scalar>(S, ansatz_space));
    std::cout << "move assignment " << sw.toc() << std::endl;
    sw.tic();

    Bembel::BlockClusterTree<Scalar, Scalar> bct2 = bct0;
