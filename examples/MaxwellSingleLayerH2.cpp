...
int main() {
  ...
  const std::function<VectorX<std::complex<Scalar>>(Vector3<Scalar>)> fun = [wavenumber](Vector3<Scalar> pt) {
  ...
  for (int polynomial_degree = 1; polynomial_degree < polynomial_degree_max + 1;
    ...
    for (int refinement_level = 0; refinement_level < refinement_level_max + 1;
      ...
      DiscreteOperator<H2Matrix<std::complex<Scalar>, Scalar>,
                       MaxwellSingleLayerOperator<Scalar>, Scalar>
      ...
      // solve system
      GMRES<H2Matrix<std::complex<Scalar>, Scalar>, IdentityPreconditioner> gmres;
